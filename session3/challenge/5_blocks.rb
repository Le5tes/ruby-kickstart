...
#
def spiral_access (twod_array, &block)
	return nil if twod_array.empty?
	twod_array[0].each(&block)
	spiral_access(twod_array[1...(twod_array.length)].transpose.reverse, &block)
end
